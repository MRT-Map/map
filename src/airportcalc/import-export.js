...
export function exportAirportcalc() {
    ...
    airportcalcGroup.getLayers().forEach(l => {
        ...
        if (l instanceof L.Circle) {
            ...
            feature.geometry.coordinates = latlngs.map(ll => ll.map((sll) => [sll.lat, sll.lng]));
        }
...
export function importAirportcalc(geojson) {
    geojson.features.forEach(f => {
        if (f.properties.shape == "Circle")
            airportcalcGroup.addLayer(L.circle(f.geometry.coordinates, {
                color: f.properties.color,
                radius: f.properties.radius,
            }).addTo(g().map));
        else if (f.properties.shape == "Rectangle")
            airportcalcGroup.addLayer(L.rectangle([
                f.geometry.coordinates[0],
                f.geometry.coordinates[2],
            ], { color: f.properties.color }).addTo(g().map));
        else
            airportcalcGroup.addLayer(L.polygon(f.geometry.coordinates, {
                color: f.properties.color,
            }).addTo(g().map));
    });
}
