...
export function initAirportcalc() {
    ...
    setInterval(() => {
        ...
        if (isNaN(percentage))
            percentage = 0;
        const newdata = `<img src="media/ac2-dark.png" style="height: 100%; float: right;">
...
function calcCityArea() {
    ...
    airportcalcGroup.getLayers().forEach(l => {
        ...
        if (l.options.color == "#ff0000")
            cityArea += Math.abs(newArea);
        else
            airportArea += Math.abs(newArea);
    });
