...
export async function initTownSearch() {
    // get MRT members
    const res = await fetch("https://script.google.com/macros/s/AKfycbwde4vwt0l4_-qOFK_gL2KbVAdy7iag3BID8NWu2DQ1566kJlqyAS1Y/exec?spreadsheetId=1Hhj_Cghfhfs8Xh5v5gt65kGc4mDW0sC5GWULKidOBW8&sheetName=Members");
    MRTMembers = (await res.json());
...
function townSearch(query) {
    ...
    gcm().searchLayer = L.featureGroup();
    ...
    const relevantTowns = window.mapGlobals.cityMap.towns.filter(t => t.Name?.toString().toLowerCase().includes(query.toLowerCase()) ??
    ...
    for (const town of relevantTowns) {
        ...
        if (isNaN(rawCoords[0]) || isNaN(rawCoords[2])) {
            ...
            gcm().searchLayer.addLayer(L.marker(mapcoord([rawCoords[0], rawCoords[2]]))
                .addTo(map)
...
export function focusMap(x, z) {
    if (isNaN(x) || isNaN(z))
        return alert("This town cannot be displayed because it contains invalid coordinates. Please contact a staff member to fix.");
    console.log(x, z);
