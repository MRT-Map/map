...
function townSearch(query: string) {
  ...
  window.globals.cityMap.searchLayer = L.featureGroup() as L.FeatureGroup<L.Marker>
  ...
  const relevantTowns = window.globals.cityMap.towns.filter(t => t.Name?.toString().toLowerCase().includes(query.toLowerCase()) ?? relevantNames.includes(t.Mayor.toString().toLowerCase()));
...
export function focusMap(ox: string, oz: string) {
  const x = parseInt(ox);
  const z = parseInt(oz);
