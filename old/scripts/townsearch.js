  ...
  success: (res) => {
    ...
    for (let member in MRTMembers) {
      ...
      if (MRTMembers[member]["Temporary Usernames"] == "") {
        ...
        let tempUsernames =
          MRTMembers[member]["Temporary Usernames"].split(",");
...
function townSearch(query) {
  ...
  const relevantTowns = towns.filter(
    (t) =>
  ...
  if (value == null || value == "") {
    ...
    for (const result of results) {
      document.getElementById(
        "search__results"
      ).innerHTML += `<div class="result" onclick="focusMap('${result.X.toString().trim()}', '${result.Z.toString().trim()}')"><div class="result__name">${
        result.Name
