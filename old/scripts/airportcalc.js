    ...
    if (l.pm._shape == "Circle") {
      ...
      for (i = 0; i < latlngs.length; i++) {
        for (j = 0; j < latlngs[i].length; j++)
          latlngs[i][j] = [latlngs[i][j].lat, latlngs[i][j].lng];
      }
...
function importAirportcalc(geojson) {
  geojson.features.forEach((f) => {
    if (f.properties.shape == "Circle")
      ...
      airportcalcGroup.addLayer(
        L.polygon(f.geometry.coordinates, { color: f.properties.color }).addTo(
          map
        )
      );
  });
...
setInterval(() => {
  if (bottomBar.isVisible()) {
    ...
    let newdata =
      `<img src="media/airportcalcicon.png" style="height: 100%; float: right;" title="Logo by Cortesi">
...
airportcalcButton = L.easyButton(
  "fa-ruler",
  toggleControls,
  "Open Airportcalc 2",
  { position: "topright" }
